...
class FtCtrl extends TechvpnControl
{
...
class FtClient
{
    ...
    private TechvpnUtility $utility;

    public function __construct(TechvpnUtility $utility, array $options)
    {
    ...
    public function get_utility(): TechvpnUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TechvpnContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TechvpnContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TechvpnConfig::make_feature($name);
    ...
    private static function buildUrl(TechvpnSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TechvpnError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TechvpnResponse($fetched);
        }
    ...
    private function fail(TechvpnContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TechvpnContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
