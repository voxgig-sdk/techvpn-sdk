...
// Techvpn SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class TechvpnMakeError
{
    public static function call(?TechvpnContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TechvpnContext([], null);
        }
        $op = $ctx->op ?? new TechvpnOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TechvpnResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TechvpnError) ? $err->msg : (string)$err;
        $msg = "TechvpnSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new TechvpnError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof TechvpnError) {
