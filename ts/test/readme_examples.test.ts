...
function isRunnable(code: string): boolean {
  return (
    /new\s+TechvpnSDK\b/.test(code) ||
    /\bTechvpnSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TechvpnSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TechvpnSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
