...
class ServerEntity
{
    ...
    {
        if ($args) {
            $this->_data = TechvpnHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = TechvpnHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    /**
     * List Server items matching the given filter.
     *
     * @param ServerListMatch|array|null $reqmatch Match filter (any subset
     *   of Server fields) as an assoc-array; ServerListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Server[]|array A list of Server items as assoc-arrays at
     *   the SDK boundary; throws TechvpnError on failure (item-5 convention).
     */
