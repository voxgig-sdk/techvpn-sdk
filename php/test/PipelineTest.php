...
{
    private static function utility(): TechvpnUtility
    {
        return new TechvpnUtility();
    }
    ...
    private static function ctx(array $over = []): TechvpnContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TechvpnContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TechvpnOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TechvpnError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TechvpnOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TechvpnMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TechvpnOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TechvpnMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TechvpnOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TechvpnMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TechvpnSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TechvpnMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TechvpnResponse([]),
            'result' => new TechvpnResult([]),
        ]);
        [, $err] = TechvpnMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TechvpnSpec([]),
            'response' => null,
            'result' => new TechvpnResult([]),
        ]);
        [, $err] = TechvpnMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TechvpnSpec(['step' => 's']),
            'response' => new TechvpnResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TechvpnResult([]),
        ]);
        [, $err] = TechvpnMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TechvpnSpec(['step' => 's']),
            'response' => new TechvpnResponse(self::resp(200, ['v' => 1])),
            'result' => new TechvpnResult([]),
        ]);
        [, $err] = TechvpnMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TechvpnSpec(['step' => 's']),
            'response' => new TechvpnResponse(self::resp(200, ['v' => 2])),
            'result' => new TechvpnResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TechvpnMakeResponse::call($ctx);
    ...
    {
        $preset = new TechvpnResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TechvpnSpec([]),
            'response' => new TechvpnResponse([]),
            'result' => new TechvpnResult([]),
        ...
        [$got, $err] = TechvpnMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TechvpnResult([])]);
        [, $err] = TechvpnMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TechvpnSpec([]), 'result' => null]);
        [, $err] = TechvpnMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TechvpnSpec(['step' => 's']),
            'result' => new TechvpnResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TechvpnOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TechvpnMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TechvpnSpec(['step' => 's']),
            'result' => new TechvpnResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TechvpnOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TechvpnMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TechvpnResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TechvpnSpec([]),
            'result' => new TechvpnResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TechvpnMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TechvpnMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TechvpnError('boom', 'boom');
        $utility->fetcher = function (TechvpnContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TechvpnSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TechvpnMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TechvpnContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TechvpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TechvpnResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TechvpnContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TechvpnSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TechvpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TechvpnResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TechvpnSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TechvpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TechvpnMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TechvpnMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TechvpnResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TechvpnDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TechvpnResult(['ok' => false])]);
        $this->expectException(TechvpnError::class);
        TechvpnDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TechvpnResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TechvpnMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TechvpnMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TechvpnBaseFeature();
        $b = new TechvpnBaseFeature();
        TechvpnFeatureAdd::call($ctx, $a);
        TechvpnFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TechvpnBaseFeature
    {
        $f = new TechvpnBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TechvpnFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TechvpnContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TechvpnSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TechvpnPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TechvpnResponse(['headers' => null]),
            'result' => new TechvpnResult([]),
        ]);
        TechvpnResultHeaders::call($ctx);
