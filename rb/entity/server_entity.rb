...
class ServerEntity
  ...
  def data_set(args)
    if args
      @_data = TechvpnHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = TechvpnHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [ServerListMatch, Hash, nil] match filter (any subset of
  #   Server fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Server>, Array] the matching Server items; raises TechvpnError on failure
