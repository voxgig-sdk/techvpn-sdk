...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = TechvpnOmni.make_runner(TEST_JSON_FILE, TechvpnSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : TechvpnOmni.errify(err)
    end
