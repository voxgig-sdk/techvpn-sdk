...
module TechvpnUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = TechvpnResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TechvpnResponse.new(fetched)
    else
