...
boru-driven command-line client **and** interactive REPL for the Techvpn
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/techvpn-cli)
make build
...
./techvpn-cli list server

# 5. Override the API base URL for a single call
TECHVPN_BASE=https://api.example.com ./techvpn-cli list server
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/techvpn-cli
   ```
...
Run with no arguments to open a REPL (prompt `techvpn>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/techvpn-cli`. |
...
(`./techvpn-cli <expr>`) and interactively (the REPL), and expressions compose the
