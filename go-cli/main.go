// Command Techvpn-cli is a CLI + REPL for the Techvpn SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TECHVPN_APIKEY (api key), TECHVPN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TECHVPN_APIKEY carries the API key and
	// TECHVPN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TECHVPN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TECHVPN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTechvpnSDK(opts)
