...
[MCP](https://modelcontextprotocol.io) server exposing the Techvpn SDK as
two agent tools — `techvpn_list` and `techvpn_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/techvpn-mcp)
make build

# 2. Provide credentials via the environment
export TECHVPN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user techvpn \
  -- /absolute/path/to/techvpn-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./techvpn-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// techvpn_list: first page of records
{ "entity": "server" }
{ "entity": "server", "query": { } }

// techvpn_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/techvpn-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user techvpn \
     -- "$PWD"/dist/*/techvpn-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `techvpn_list` and `techvpn_load` tools now appear
   in new sessions. Ask the agent to *"list server using techvpn"*
   and it calls `techvpn_list` with `{"entity":"server"}`.
...
```sh
export TECHVPN_APIKEY=sk_live_xxx            # API key
export TECHVPN_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `techvpn_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `techvpn_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TECHVPN_APIKEY` | API key sent with every request. |
| `TECHVPN_BASE` | Optional override of the API base URL. |
...
```sh
./techvpn-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"techvpn_load","arguments":{"entity":"server","query":{"id":1}}}}'
```
