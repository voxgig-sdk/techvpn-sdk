# Techvpn Golang SDK Reference

Complete API reference for the Techvpn Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *TechvpnSDK`
...
#### `Server(data map[string]any) TechvpnEntity`
...
| `load` | `float64` | No | Current server load percentage |
...
```go
results, err := client.Server(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(results)
