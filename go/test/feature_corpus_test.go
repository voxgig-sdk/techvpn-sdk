...
//
// NewTechvpnSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.TechvpnSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
...
func fcCandidates(client *sdk.TechvpnSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.TechvpnSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.TechvpnSDK, name string) any {
