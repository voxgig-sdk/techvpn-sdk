...
import (
	...
	omni "github.com/voxgig-sdk/techvpn-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TechvpnError); is {
				out := map[string]any{
					"name":    "TechvpnError",
...
func omniCtx(arg any, client *sdk.TechvpnSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TechvpnSDK); is {
			client = live
