...
TechVPN API client, generated from the OpenAPI spec.

The TechVPN API provides access to functionalities of the techvpn.cloud VPN browser extension, allowing users to connect swiftly without the need for accounts, ads, or data collection.

Learn more about TechVPN API at [github.com/l0v3m0n3y/techvpn](https://github.com/l0v3m0n3y/techvpn).

This is an unofficial SDK for the TechVPN public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
const items = await client.Server().list()
```
...
```go
client := sdk.Test()
result, err := client.Server(nil).List(
    nil, nil,
)
```
...
```bash
cd go-mcp && go build -o techvpn-mcp .
```
...
```json
{
  "mcpServers": {
    "techvpn": {
      "command": "/abs/path/to/techvpn-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **Server** | The Server entity (list). | `/api/extension/servers` |
...
# List all servers (returns a list, raises on error)
servers = client.Server().list()
...
// List all servers (returns an array; throws on error)
$servers = $client->Server()->list();
print_r(array_map(fn($item) => $item->data_get(), $servers));
```

### Golang

```go
import sdk "github.com/voxgig-sdk/techvpn-sdk/go"
...
# List all servers (returns an Array; raises on error)
servers = client.Server.list
puts servers
```
...
Generated from the TechVPN API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
