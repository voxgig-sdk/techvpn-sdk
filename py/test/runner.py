# Techvpn SDK test runner

from __future__ import annotations
import os
import json

from techvpn_sdk.utility.voxgig_struct import voxgig_struct as vs


class TechvpnTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TechvpnTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TechvpnTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TechvpnTestRunner.getenv("TECHVPN_TEST_LIVE")
        override = TechvpnTestRunner.getenv("TECHVPN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TechvpnTestRunner.getenv(key)
        ...
        explain = TechvpnTestRunner.getenv("TECHVPN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TECHVPN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TechvpnTestRunner._test_control is not None:
            return TechvpnTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TechvpnTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TechvpnTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TechvpnTestRunner.load_test_control()
...
def load_env_local():
    TechvpnTestRunner.load_env_local()


def env_override(m):
    return TechvpnTestRunner.env_override(m)


def entity_data(v):
    return TechvpnTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TechvpnTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TechvpnTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TechvpnTestRunner.load_test_control()


def live_client_options():
    return TechvpnTestRunner.live_client_options()


def live_delay_ms():
    return TechvpnTestRunner.live_delay_ms()
